...
function FacultyLogin() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const role = await loginUserWithRole(email, password, "faculty");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <form onSubmit={handleLogin} className="bg-white p-8 rounded shadow-md w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4 text-center text-green-600">Faculty Login</h2>
        ...
        <p className="text-sm text-center mt-4">
          ...
          <Link to="/signup/faculty" className="text-green-600 hover:underline">Sign up here</Link>
        </p>
      </form>
    </div>
  );
